...
# c - a variable representing the price for the specific category
# output:
# node - a vector like (index, value) indicating the node that would give the minimum SSE if we split the data there

fnd_node <- function(P, X, Y, c)
{
  node = NaN
  min_sse = get_sse(P, c, Y)
  for (i in 1:length(X)) {
    index = i
    X_vector = X[[i]]
    for (j in 1:length(X_vector)){
      sse = get_split_sse(X_vector, Y, j, c)
      if (sse < min_sse){
        min_sse = sse
        value = X_vector[j]
        node = c(index, value)
